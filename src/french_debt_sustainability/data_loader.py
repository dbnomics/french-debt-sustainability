...
def download_maa_debt_percent():
    maa_debt_percent = fetch_series(
        "INSEE/DETTE-TRIM-APU-2014/T.DETTE_MAASTRICHT.S13.F.PROPORTION.FE.POURCENT.BRUT.2014.TRUE"
    )
    return maa_debt_percent


def download_maa_debt():
    maa_debt = fetch_series(
        "INSEE/DETTE-TRIM-APU-2014/T.DETTE_MAASTRICHT.S13.F.VALEUR_ABSOLUE.FE.EUROS.BRUT.2014.TRUE"
    )
    return maa_debt
...
def transform_data_expenditures(expend):
    new_expend = expend.pivot_table(
        values="value", index="Categories", columns="original_period"
    )
    ...
    original_expend = pd.melt(
        reset_expend,
        id_vars=["Categories"],
        var_name="original_period",
        value_name="value",
    )
...
def download_hh_data():
    hh_df = fetch_series(
        ["OECD/NAAG/FRA.DBTS14_S15NDI", "OECD/DP_LIVE/FRA.HHSAV.TOT.PC_HHDI.A"]
    )
    return hh_df
